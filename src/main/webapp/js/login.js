...
$(document).ready(function() {
	...
	let isemail = false;
	let isphone = false;
	let isfirstPartValid = false;
	let isIdentifyPartValid = false;
	// 폼 제출 버튼 클릭 핸들러
	$("#signup_form").on("click", function(event) {
		if (!isid) {
			...
			alert("주민등록번호 앞자리 6개를 제대로 입력하여 주세요!");
			event.preventDefault();
		} else if (!isIdentifyPartValid) {
			alert("주민등록번호 뒷자리 1개를 제대로 입력하여 주세요!");
			event.preventDefault();
		}else if (!isemail) {
	...
	$("#input_first").on("input", function() {
		...
		isfirstPartValid = (firstPart.length === 6);
	});
	...
	$("#input_identify").on("input", function() {
		...
		isIdentifyPartValid = (identifyPart.length === 1);
	});
